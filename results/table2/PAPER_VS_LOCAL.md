...
## Column-header caveats (read before comparing numbers)

- **ARC column.** Paper Table 2's "ARC" header refers to **ARC-AGI-Text**
  (private split, not publicly released). Every reviewer-side
  reproduction in this repository instead evaluates **AI2 ARC-Challenge
  (text MCQ, 1,172 problems)** as a documented proxy benchmark; see
  [`LIMITATIONS.md`](../../LIMITATIONS.md) §3 + §13. The two benchmarks
  share a name but differ in difficulty distribution, so the local 80 %
  number is a property of the proxy and should not be compared
  side-by-side with the paper's 36.1–57.8 % ARC-AGI-Text range.
- **HumanEval column.** Paper §7.1 footnote marks HumanEval as
  *relative-comparison-only* (pretraining exposure); absolute pass@1
  numbers across implementations are not expected to align.
